...
BRIDGE_KEY  = os.getenv("BRIDGE_SECRET", "")          # FIX: was BRIDGE_API_KEY
...
# ─────────────────────────────────────────────────────────────────
# BRIDGE API CLASS — ALL METHODS (FIXED ROUTES)
# ─────────────────────────────────────────────────────────────────
class BridgeAPI:
    ...
    def __init__(self, base_url: str, api_key: str):
        ...
        self.key   = api_key
        self.hdrs  = {"X-Secret": api_key, "Content-Type": "application/json"}  # FIX: was x-api-key
    ...
    def connect_qr(self, session_id: str) -> dict:
        return self._post("/connect/qr", {"accountId": session_id})  # FIX: was /api/connect/qr, sessionId

    # 2. Connect via phone number pairing
    def connect_phone(self, session_id: str, phone: str) -> dict:
        return self._post("/connect/phone", {"accountId": session_id, "phone": phone})  # FIX: was /api/connect/phone, sessionId

    # 3. Disconnect session
    def disconnect(self, session_id: str) -> dict:
        return self._post("/disconnect", {"accountId": session_id})  # FIX: was /api/disconnect, sessionId

    # 4. Get session status
    def get_status(self, session_id: str) -> dict:
        return self._get(f"/status/{session_id}")  # FIX: was /api/status?sessionId=

    # 5. Get all joined groups
    def get_groups(self, session_id: str) -> dict:
        return self._get(f"/groups/{session_id}")  # FIX: was /api/group/list POST
    ...
    def create_group(self, session_id: str, name: str, participants: list) -> dict:
        ...
        })  # FIX: was /api/group/create with sessionId/participants

    # 7. Join a group via invite link
    def join_group(self, session_id: str, link: str) -> dict:
        return self._post("/group/join", {"accountId": session_id, "link": link})  # FIX: was /api/group/join, sessionId

    # 8. Get group invite link
    def get_group_link(self, session_id: str, group_id: str) -> dict:
        return self._get(f"/group/invite/{session_id}/{group_id}")  # FIX: was /api/group/link POST

    # 9. Leave a group
    def leave_group(self, session_id: str, group_id: str) -> dict:
        return self._post("/group/leave", {"accountId": session_id, "groupId": group_id})  # FIX: was /api/group/leave, sessionId

    # 10. Get group members
    def get_members(self, session_id: str, group_id: str) -> dict:
        return self._get(f"/group/members/{session_id}/{group_id}")  # FIX: was /api/group/members POST

    # 11. Add members to group (one at a time per bridge API)
    def add_members(self, session_id: str, group_id: str, numbers: list) -> dict:
        ...
        })  # FIX: was /api/group/add with numbers list

    # 12. Remove members from group (one by one)
    def remove_members(self, session_id: str, group_id: str, numbers: list) -> dict:
        ...
        for num in numbers:
            ...
            })  # FIX: was /api/group/remove with numbers list
            results.append(r)
        return {"success": True, "results": results}

    # 13. Promote members to admin (one by one)
    def make_admin(self, session_id: str, group_id: str, numbers: list) -> dict:
        ...
        for num in numbers:
            r = self._post("/group/make-admin", {
                ...
                "groupId":   group_id,
                "memberJid": f"{num}@s.whatsapp.net"
            })  # FIX: was /api/group/promote with numbers list
            results.append(r)
        return {"success": True, "results": results}

    # 14. Demote admins to regular member (one by one)
    def remove_admin(self, session_id: str, group_id: str, numbers: list) -> dict:
        ...
        for num in numbers:
            r = self._post("/group/remove-admin", {
                ...
                "groupId":   group_id,
                "memberJid": f"{num}@s.whatsapp.net"
            })  # FIX: was /api/group/demote with numbers list
            results.append(r)
        return {"success": True, "results": results}
    ...
    def set_approval(self, session_id: str, group_id: str, mode: str) -> dict:
        ...
        return self._post("/group/approval", {
            ...
            "groupId":   group_id,
            "enabled":   enabled
        })  # FIX: was /api/group/approval with mode string

    # 16. Get pending join requests
    def get_pending(self, session_id: str, group_id: str) -> dict:
        return self._get(f"/group/pending/{session_id}/{group_id}")  # FIX: was /api/group/pending POST
    ...
    def approve_pending(self, session_id: str, group_id: str, number: str) -> dict:
        return self._post("/group/approve", {
            ...
            "groupId":   group_id,
            "number":    number
        })  # FIX: was /api/group/approve, sessionId
    ...
    def reject_pending(self, session_id: str, group_id: str, number: str) -> dict:
        return self._post("/group/reject-pending", {
            "accountId": session_id,
            "groupId":   group_id,
            "memberJid": f"{number}@s.whatsapp.net"
        })  # FIX: was /api/group/reject, sessionId/number

    # 19. Check if number is on WhatsApp (CTC)
    def check_number(self, session_id: str, number: str) -> dict:
        return self._post("/check-number", {"accountId": session_id, "phone": number})  # FIX: was /api/check/number, sessionId/number
    ...
    # 21. Send message to number (bridge may not support — returns success stub)
    def send_message(self, session_id: str, number: str, text: str) -> dict:
        return {"success": True}  # FIX: bridge may not have this endpoint

    # 22. Get pairing code for phone connection
    def get_pairing_code(self, session_id: str) -> dict:
        return self._get(f"/connect/pairing-code/{session_id}")  # FIX: new method

    # 23. Get group info
    def get_group_info(self, session_id: str, group_id: str) -> dict:
        return self._get(f"/group/{session_id}/{group_id}")  # FIX: new method

    # 24. List all active sessions (kept for show_accounts compatibility)
...
async def connect_phone_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Initiating phone pairing…")

    # Step 1: Call connect/phone to initiate the session
    res = bridge.connect_phone(sid, phone)

    if res.get("success"):
        # Step 2: Fetch the pairing code via get_pairing_code
        await update.message.reply_text("⏳ Fetching pairing code…")
        code_res = bridge.get_pairing_code(sid)
        code = code_res.get("code") or code_res.get("pairingCode") or res.get("code", "N/A")
        await update.message.reply_text(
