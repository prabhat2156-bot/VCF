...
async def qv_more(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "➕ Add More":
        await update.message.reply_text(
            "📞 Enter more phone number(s) (one per line / comma / space separated):",
            reply_markup=back_cancel_kb(),
        )
        return QV_PHONE
    elif text == "✅ Finish":
...
# Common WhatsApp/group UI labels we should never confuse with a group name.
_WHATSAPP_UI_BLACKLIST = {
    "message", "messages", "audio", "video", "add", "call", "calls",
    "notifications", "all", "media", "visibility", "encryption",
    "encrypted", "tap", "to", "learn", "more", "and", "are",
    "end-to-end", "endto-end", "chat", "lock", "hide", "this", "on",
    "device", "advanced", "privacy", "off", "members", "member",
    "pending", "requests", "request", "you", "hey", "there",
    "i", "am", "using", "whatsapp", "group", "admin", "search",
    "info", "exit", "report", "block", "save", "share", "mute",
    "custom", "starred", "files", "links", "wallpaper",
}


def _looks_like_group_name(token: str) -> bool:
    if not token:
        return False
    if len(token) < 2:
        return False
    low = token.lower()
    if low in _WHATSAPP_UI_BLACKLIST:
        return False
...
def extract_group_info(ocr_text: str) -> Tuple[str, str]:
    """Extract (group_name, pending_count) from a WhatsApp group screenshot OCR.

    Returns ('', '') if nothing usable is found.
    """
    raw_lines = [l.rstrip() for l in ocr_text.splitlines()]
    lines = [l.strip() for l in raw_lines if l.strip()]

    # --- Pending Requests count -------------------------------------------
    pending = ""
    for i, line in enumerate(lines):
        if re.search(r'pending\s*request', line, re.IGNORECASE):
            # number on same line
            m = re.search(r'(\d{1,6})', line)
            if m:
                pending = m.group(1)
                break
            # number on next line
            if i + 1 < len(lines):
                m2 = re.search(r'(\d{1,6})', lines[i + 1])
                if m2:
                    pending = m2.group(1)
                    break

    # --- Group name -------------------------------------------------------
    # Strategy: WhatsApp shows the group title near the top, after the back
    # arrow. Walk the first ~10 lines and pick the first short alphanumeric
    # token that is not a known UI label.
    name = ""
    for line in lines[:12]:
        # skip lines that look like UI rows (e.g. "Notifications All")
        # try each whitespace-separated token
        tokens = re.findall(r'[A-Za-z0-9][A-Za-z0-9_\-]{1,30}', line)
        for tok in tokens:
            if _looks_like_group_name(tok):
                # prefer tokens that mix letters & digits (typical group IDs)
                # or are all letters with len>=3
...
def parse_group_list(ocr_text: str) -> List[Tuple[str, str]]:
    """Legacy multi-row list parser — used as a fallback when the screenshot
    contains a *list* of groups (not a single group settings page)."""
    results = []
    tier1 = re.findall(
    ...
    try:
        ...
        for img_bytes in images:
            try:
                ...
                # First try the single-group-page extraction (one screenshot
                # = one group settings page → one row).
                name, pending = extract_group_info(text)
                if name or pending:
                    all_groups.append((name or "Unknown", pending or "0"))
                else:
                    # Fallback: maybe the screenshot shows a list of groups.
        ...
        lines = ["📋 *Group List* — Name & Pending Requests\n"]
        txt_lines = ["Group List — Name & Pending Requests", ""]
        for i, (name, count) in enumerate(all_groups, 1):
            lines.append(f"`{i}.` *{name}* — Pending: `{count}`")
            txt_lines.append(f"{i}. {name} — Pending: {count}")
...
def build_st_conv() -> ConversationHandler:
    return ConversationHandler(
        ...
        states={
            ...
            ST_SET_BASENAME:     [MessageHandler(filters.TEXT & ~filters.COMMAND, st_set_basename)],
            ST_SET_FILE_START:   [MessageHandler(filters.TEXT & ~filters.COMMAND, st_set_file_start)],
            ST_SET_GROUP_NAME:   [MessageHandler(filters.TEXT & ~filters.COMMAND, st_set_group_name)],
            ST_SET_GROUP_START:  [MessageHandler(filters.TEXT & ~filters.COMMAND, st_set_group_start)],
        },
        fallbacks=COMMON_FALLBACKS,
        allow_reentry=True,
        per_message=False,
